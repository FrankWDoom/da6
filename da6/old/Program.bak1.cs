...
namespace da6
{
    ...
    class Program
    {
        ...
        public static int hexdec(string hex_string)
        {
            ...
            int i = int.Parse(hex_string.Replace("$", ""), System.Globalization.NumberStyles.HexNumber);
            return i;
        }
        ...
        public static string str_pad(string input, int pad_length, char pad_string = ' ', PhpStrPadType pad_type = PhpStrPadType.STR_PAD_RIGHT)
        {
            ...
            switch (pad_type)
            {
                ...
                case PhpStrPadType.STR_PAD_BOTH:
                    ...
                    // todo maybe reverse left/right order if not coming out right
                    //return str_pad(
        ...
        public static string str_pad(string input, int pad_length, string pad_string, PhpStrPadType pad_type = PhpStrPadType.STR_PAD_RIGHT)
        {
            ...
            if (pad_string.Length > 1)
                throw new ArgumentException("pad value has to be single character", "pad_string"); // todo
        ...
        */
        ...
        private static Dictionary<object, object> readLabelText(string str)
        {
            ...
            var matches = Regex.Match(str,
                @"^\s*([a-zA-Z0-9_\-\+\@]*)\s*\=\s*([\$\%]*)([a-fA-F0-9]*)", RegexOptions.Multiline);
...
");
        ...
        public static void Run(string[] argv)
        {
            ...
            if (cdlFilename != null)
            {
                cdlFile = System.IO.File.OpenRead(cdlFilename); //cdlFile = fopen(cdlFilename, 'r');
                cdlByte = 0;
            }
            ...
            theText.Append(commentHeader(pathinfo(filename, PhpPathInfo.PATHINFO_BASENAME) + " disasembled by DISASM6(dotnet) v" + VERSION, false));
            //invalidCounter = 0;
            ...
            while (pass <= lastPass)
            {
                ...
                if (pass == 1)
                {
                    ...
                    if (!noDetect)
                    {
                        ...
                        if (headerInfo?.prg == 2)
                        {
                            ...
                            if (BytesEqual(prg0, prg1) && headerInfo.mapper == 0)
                            {
                ...
                while (!feof(file) && counter < fileLength)
                {
                    ...
                    if (cdlFilename != null)
                    {
                        ...
                        else if (oldPrgLabels != null && isset(oldPrgLabels, dechex_pad(counter)))
                        {
                            cdlByte = bindec("00000001");
                        }
                        // if byte is zero and we're at a label, but not program, assume data (only on 2nd pass)
                        else if (isset(oldLabels, dechex_pad(counter)) && pass > 1)
                        {
                        ...
                        // data byte
                        if ((cdlByte & bindec("00000010")) != 0 && (cdlByte & 1) == 0)
                        {
                        ...
                        if (pass >= 1)
                        {
                            ...
                            for (var i = 1; i <= readBytes; i++)
                            {
                                if (isCounterLabel(counter + i, oldLabels)
                                   ...
                                   || (usingMapper2 && headerInfo?.mapper == 2 && counter + i > 0xBFFF && prgBank < headerInfo.prg - 1)
                    ...
                    if (isset(oldLabels, byteStr) && lbl != "")
                    {
                        ...
                        newByteStr = (oldLabel is bool && ((bool)oldLabel == true))
                           ? newByteStr
                           : (string)oldLabel;
                    }
                    ...
                    if (pass == lastPass)
                    {
                        ...
                        if (oldLabels.ContainsKey(counter_pad))
                        {
                            ...
                            {
                                leng = ((string[])oldLabels[counter_pad]).Length;
                            ...
                            for (var i = 0; i < leng; i++)
                            {
                                if (is_array(oldLabels[counter_pad]))
                                {
                                    var arr = (string[])oldLabels[counter_pad];
                                    theLabel = arr[i];
                                }
                                else
                                {
                                ...
                                {
                                    case "irq":
                                        theText.Append(commentHeader("irq/brk vector", !(oldDidDrawLine || didDrawLine), !(oldDidDrawLine || didDrawLine)));
                                        break;

                                    case "nmi":
                                    case "reset":
                                        theText.Append(commentHeader($"{theLabel} vector", !(oldDidDrawLine || didDrawLine), !(oldDidDrawLine || didDrawLine)));
                                        break;
                ...
                if (pass < lastPass && oldLabels != null && DictionariesEqual(labels, oldLabels)) // todo labels should not equal oldlabels
                {
                    lastPass = pass + 1;
                }
