...
");
        ...
        public static void Run(Inputs inputs)
        {
            ...
            if (fileEndOverride)
            {
                fileLength = fileStart + fileEnd; // todo seems wrong
                lengthOverride = true;
            }
            ...
            if (cdlFilename != null)
            {
                rawCdl = File.ReadAllBytes(cdlFilename);
                //cdlFile = File.OpenRead(cdlFilename); //cdlFile = fopen(cdlFilename, 'r'); // todo remove
                cdlByte = 0;
            }
            ...
            if (!ignoreHeader)
            {
                ...
                if (headerInfo.chr > 0)
                {
                    rawChr = new byte[headerInfo.chr * 0x2000]; // 8K  
            ...
            if (rawPrg.Length > 0x8000 && !usingMapper2)
            {
                Console.WriteLine("PRG length requires a mapper, use mapper 2 option"); // todo auto
                return;
            }
            ...
            while (pass <= lastPass)
            {
                ...
                {
                    labels = new Dictionary<object, object>(initLabels);
                }
                ...
                if (pass == 1)
                {
                    ...
                    if (!noDetect)
                    {
                        ...
                        if (headerInfo?.prg == 2) // 2 x 16K
                        {
                            ...
                            if (headerInfo.mapper == 0 && IsDataRepeated(slice)) // todo cnrom could also be this
                            {
                    ...
                    if (codeStartOverride)
                    {
                        ...
                        echo("Starting at code location $" + dechex_pad(codeStart) + "\n");
                    }
            ...
            if (includeChr && headerInfo != null)
            {
                ...
                File.WriteAllBytes(shortname + ".chr", rawChr);
