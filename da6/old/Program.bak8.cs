...
namespace da6
{
    ...
    class Program
    {
        ...
        static void Main(string[] argv)
        {
            ...
            // parameters start with filename, index=0
            ...
            var filename  = _filename  = argv[0];
            if (!File.Exists(filename))
            {
                // first arg not a filename
            ...
            _originalArgs = args.ToString();
            RunOriginal(argv);

            time_start = microtime(true);

            var inputs = GetArgs(argv);
            Run(inputs);
        ...
        private static bool isValidLabel(string addr)
        {
            ...
            return (newaddr >= origin && newaddr < endOfCode);
        }
        ...
        ///// <param name="sliceStart">inclusive, ie 0x8000</param>
        ///// <param name="sliceEnd">inclusive, ie 0xFFF9</param>
        ///// <returns></returns>
        //private static bool isValidLabel(string addr, int sliceStart /* = origin */, int sliceEnd /* = 0xFFFA */)
        //{
        //    var newaddr = hexdec(addr);
        //    return (newaddr >= sliceStart && newaddr <= sliceEnd);
        //}
        ...
        private static string wordStr(byte[] str, int offset = 0)
        {
            return dechex_pad(str[1 + offset]) + dechex_pad(str[0 + offset]);
        }
        ...
        //private static HeaderInfo getHeaderInfo(FileStream file)
        //{
        ...
        private static HeaderInfo getHeaderInfo(byte[] data)
        {
            int index = 0;

            //var head = fread(file, 0x4);
            var head = new byte[4];
            Array.Copy(data, index, head, 0, head.Length);
            index += head.Length;

            var nes = Encoding.ASCII.GetString(head);
            var tag = ("NES" + ((char)0x1A)).ToString();

            if (!string.Equals(nes, tag))
            {
            ...
            info.head = head;
            info.prg = data[index++]; // (fread(file, 1))[0];
            info.chr = data[index++]; // (fread(file, 1))[0];
            info.ctrl_1 = data[index++]; // (fread(file, 1))[0];
            info.ctrl_2 = data[index++]; // (fread(file, 1))[0];
            info.tail = new byte[8]; // fread(file, 8);
            Array.Copy(data, index, info.tail, 0, info.tail.Length);
        ...
        private static string processHeaderInfo(HeaderInfo info)
        {
            ...
            if (info != null)
            {
                ...
                string series = LEFT_MARGIN + ".hex " + string.Join(" ", info.tail.Take(4).Select(b => b.ToString("x2")));
                sb.Append(series.PadRight(pad));
                sb.AppendLine(" ; iNES Tail ");

                series = LEFT_MARGIN + ".hex " + string.Join(" ", info.tail.Skip(4).Select(b => b.ToString("x2")));
                sb.Append(series.PadRight(pad));
                sb.AppendLine("  ");
        ...
        private static Dictionary<object, object> readLabelText(string[] lines)
        {
            ...
            foreach (var line in lines)
            {
                ...
                var key = dechex_pad(addrVal);
                string[] labels;

                if (arr.ContainsKey(key))
                {
                    labels = (string[])arr[key];
                }
                else
                {
                    labels = new string[0];
                ...
                var tlist = labels.ToList();
                tlist.Add(label.ToLower());
                arr[key] = tlist.ToArray();
        ...
        private static string outputLabels(Dictionary<object, object> arr, string text)
        {
            ...
            foreach (var item in arr)
            {
                ...
                var valArr = item.Value as string[];
                if (valArr != null)
                {
                    labels.AddRange(valArr);
                }
                else
                {
...
");
        ...
        private static Inputs GetArgs(string[] argv)
        {
        ...
        private static Dictionary<object, object> LoadFileLabels(string labelFile)
        {
        ...
        private static bool IsDataRepeated(byte[] slice)
        {
            if (slice.Length % 2 != 0)
        ...
        private static string FormatOp(
            string byteStr2, string newByteStr, int addressingType, bool isInvalid, int readBytes, Dictionary<object, object> prgLabels)
        {
        ...
        private static void PrintInputs(Inputs inputs, int origin, HeaderInfo headerInfo)
        {
            ...
            if (headerInfo != null)
            {
                echo("NES Header Found - " + (inputs.showHeader ? "included in disassembly" : "not included") + "\n");
            }

            if (inputs.labelFile != null)
            {
                echo("Using user defined labels\n");
            }

            if (inputs.includeReg)
            {
                echo("Using NES registers\n");
            }

            if (inputs.cdlFilename != null)
            {
                echo("Using code/data log\n");
            }

            if (inputs.ignoreWrites != false)
            {
        ...
        private static bool[] TracePrg(byte[] slice, BankInfo bankInfo, List<int> entryPoints, byte[] cdl)
        {
            var sb = new StringBuilder();
            ...
            {
                int addr = entryPoints[ep];
                ep++;

                sb.Append($"tracing at address ${addr:X4}...");

                //if (addr < bankInfo.Origin || addr > bankInfo.EndOfBank)
                if (!bankInfo.AddressInRange(addr))
                {
                ...
                if (cdl != null)
                {
                    ...
                    if ((c & (CDL_CODE | CDL_IND_CODE)) == 0)
                    {
                        ...
                        if ((c & (CDL_DATA | CDL_IND_DATA | CDL_PCM_DATA)) != 0)
                        {
                ...
                var exitTo = new List<int>(); // todo

                while (true)
                {
                    // assume continuing code until stopping point hit
                    ...
                    if (cmd.Legal) // legal = illegal? todo
                    {
                        sb.Append($"  ${addr:X4} illegal opcode {cmd.Text}(0x{opcode:X2})   "); sb.Append(Environment.NewLine);
                    ...
                    {
                        // todo check for load/store to addresses in entry points list for suspected data
                    ...
                    if (jumps.Contains(cmd.Text)) // op = jump/return
                    {
                        ...
                        if (opcode == 0x20)
                        {
                            // jsr, which will continue here after executing
                            // add new entry point for continuation

                            var target = addr + cmd.Bytes;
                            if (!entryPoints.Contains(target))
                            {
                        ...
                        // others are end-of-execution in this line
                        break;
                    }

                    //   branches

                    if (branches.Contains(cmd.Text)) // op = jump/return
                    {
                        ...
                        if (delta == 0xff)
                        {
                            ...
                            sb.Append($"encountered branch into self at ${addr:X4}" + Environment.NewLine);
                        }
                        else if (delta == 0xfe)
                        {
                    ...
                    addr += cmd.Bytes; // length of op + operand 
                    if (cease || addr > bankInfo.EndOfBank)
                    {
        ...
        public static void Run(Inputs inputs)
        {
            ...
            var head = "DISASM6(dotnet) v" + VERSION + " - A NES-oriented 6502 disassembler - Created by Frantik 2015 - ported by FrankWDoom";
            echo($"\n{head}\n" + str_repeat('-', 79) + "\n");

            var theText = new StringBuilder();
            theText.AppendLine($"; {_originalArgs}");
            ...
            var fileLabels = LoadFileLabels(inputs.labelFile);
            if (fileLabels != null)
            {
                foreach (var item in fileLabels)
                {
            ...
            var oldLabels = new Dictionary<object, object>(initLabels); // previous-pass general labels
            var oldPrgLabels = new Dictionary<object, object>(); // previous-pass prg labels, empty for first pass
            ...
            if (inputs.cdlFilename != null)
            {
                rawCdl = File.ReadAllBytes(inputs.cdlFilename);
                cdlByte = 0;
            }
            ...
            byte oldPrg = 0;
            int prgStart = 0; // start index for prg within file, 0 or 0x10

            byte[] rawPrg = null;
            byte[] rawChr = null;


            var data = File.ReadAllBytes(inputs.filename);

            // todo check for prg repeated as a whole
            ...
            if (!inputs.ignoreHeader)
            {
                ...
                rawPrg = new byte[headerInfo.prg * 0x4000]; // 16K  
                Array.Copy(data, prgStart, rawPrg, 0, rawPrg.Length);

                if (headerInfo.chr > 0)
                {
                    rawChr = new byte[headerInfo.chr * 0x2000]; // x 8K  
                    Array.Copy(data, prgStart + rawPrg.Length, rawChr, 0, rawChr.Length);
                }
            }
            else
            {
            ...
            if (rawPrg.Length > 0x8000 && !inputs.usingMapper2)
            {
                echo("PRG length requires a mapper, use mapper 2 option"); // todo auto
                return;
            }

            List<byte[]> prgSlices = headerInfo.SlicePrg(rawPrg); // break prg down to bank-sized chunks
            List<byte[]> cdlSlices = headerInfo.SliceCdl(rawCdl, prgSlices); // break cdl down to match prg slices;
            ...
            // those would be tought to process as a whole anyway
            var slice = prgSlices.Last();

            string nmi = wordStr(slice, slice.Length - 6);
            string reset = wordStr(slice, slice.Length - 4);
            string irq = wordStr(slice, slice.Length - 2);

            var oldDidDrawLine = false;


            // check for 16k roms
            if (!inputs.noDetect && headerInfo != null)
            {
                if (headerInfo.prg == 2) // 2 x 16K
                {
                    // this would be a 16K prg doubled to 32K
                    // todo this scenario is unlikely but it's possible for a doubled 8K prg  

                    if (headerInfo.mapper == 0 && IsDataRepeated(prgSlices[0])) // todo cnrom could also be this
                    {
                        // todo re-split prg slices?
                ...
                {
                    // this would be standard 16K rom
                    //origin = originOverride ? origin : 0xc000;
                    if (!originOverride)
                    {
                        origin = 0xc000;
            ...
            entryPoints.Add(hexdec(reset));
            entryPoints.Add(hexdec(nmi));
            entryPoints.Add(hexdec(irq));
            ...
            while (pass <= lastPass)
            {
                ...
                var labels = new Dictionary<object, object>(initLabels);
                var prgLabels = new Dictionary<object, object>(initLabels);
                ...
                // todo figure out what to do with start overrides

                //if (fileStartOverride && !inputs.codeStartOverride)
                //{
                ...
                if (pass == 1)
                {
                    ...
                    if (inputs.codeStartOverride)
                    {
                        fileStart = inputs.codeStart - 0x8000; // $8000 is start of prg address space, so calculate relative to that
                        fileStartOverride = true;

                        origin = inputs.codeStart;
                        originOverride = true;

                        cdlOffset += fileStart; // cdl does not have 16 byte header

                        prgStartIndex = fileStart;

                        echo("Starting at code location $" + dechex_pad(inputs.codeStart) + "\n");
                    }
                    else if (fileStartOverride /* && !inputs.codeStartOverride */)
                    {
                    ...
                    if (lengthOverride)
                    {
                        ...
                        // filelength is relative to counter, not file 0
                    }


                    if (inputs.includeChr && headerInfo != null)
                    {
                        //echo "Using CHR-ROM\n";
                    }
                ...
                if (pass == lastPass)
                {
                    ...
                    if (header != null && inputs.showHeader)
                    {
                ...
                var branches = new[] { "BCC", "BCS", "BEQ", "BMI", "BNE", "BPL", "BVC", "BVS", };

                //while (!feof(file) && counter < fileLength)
                while (index < slice.Length && counter < fileLength)
                {
                    ...
                    )
                    {
                        ...
                        if (pass == lastPass)
                        {
                            ...
                            theText.AppendLine(LEFT_MARGIN + ".base 0x8000");
                    ...
                    if (pass < lastPass && counter == 0xFFFA)
                    {
                        ...
                        addVector(irq, "irq", labels);

                        // these are really Add() actions right?
                        prgLabels[nmi] = true;
                    ...
                    {
                        theText.Append(processVectors(nmi, reset, irq));
                        index += 6;
                        counter += 6;
                        continue;
                    }
                    ...
                    if (inputs.cdlFilename != null)
                    {
                        ...
                        if ((cdlByte & CDL_DATA) != 0 && (cdlByte & CDL_CODE) == 0)
                        {
                            ...
                            if (isCounterLabel(counter, oldLabels))
                            {
                                ...
                                if (arr != null)
                                {
                                    theOldLabel = arr[0]; // todo join?
                                }
                                else
                                {
                                    var s = oldLabels[counter_pad] as string;
                                    theOldLabel = s ?? "__" + counter_pad;
                                }
                    ...
                    var byteStr = "";
                    var byteArr = new List<string>() { dechex_pad(opcode) };
                    ...
                    if (readBytes > 0)
                    {
                        ...
                        for (var i = 1; i <= readBytes; i++)
                        {
                            ...
                            if (isCounterLabel(counterNext, oldLabels)
                               ...
                               || (operand == 0xff && branches.Contains(mnemonic)) // branch into self
                               || (inputs.usingMapper2 && headerInfo?.mapper == 2 && counterNext > 0xBFFF && prgBank < headerInfo.prg - 1)
                            ...
                            if (inputs.cdlFilename != null && isDataByte)
                            {
                                var newCdlByte = sliceCdl[index + i];
                        ...
                        if (readBytes > 0) // if readbytes is still > 0 after above
                        {
                            ...
                            Array.Copy(slice, index + 1, bytes, 0, readBytes);

                            byteArr.AddRange(bytes.Select(b => dechex_pad(b)));

                            if (addressingType == 12) // jump table ?
                            {
                            ...
                            else if (addressingType == 11) // rts table
                            {
                                byteStr = (byteArr.Count > 1 ? byteArr[1] : "") + byteArr[0];
                            }
                            else
                    ...
                    if (addressingType > 0
                      ...
                      && !(inputs.ignoreWrites && substr(mnemonic, 0, 2) == "ST" && (hexdec(byteStr) < 0x8000))) // do not add labels when writing to PRG 
                    {
                        lbl = "__";

                        //if (pass < lastPass && isInvalid != true)
                        if (pass < lastPass && !isInvalid)
                        {
                    ...
                    if (isset(oldLabels, byteStr) && lbl != "")
                    {
                        ...
                        // todo
                        //var arr = oldLabels[byteStr] as string[];
                        //var s = oldLabels[byteStr] as string;
                        //if (arr != null)
                        //{
                        //    newByteStr = arr[0];
                        //}
                    ...
                    // byteStr is raw hex value going in
                    // newByteStr is like $byteStr or a label name

                    byteStr = FormatOp(byteStr, newByteStr, addressingType, isInvalid, readBytes, prgLabels);
                    ...
                    switch (mnemonic)
                    {
                        ...
                        case "BVS":
                            ...
                            if (!isInvalidBranch && isValidLabel(addr))
                            {
                                ...
                                if (isset(labels, addr))
                                {
                                    var arr = labels[addr] as string[];
                                    var s = labels[addr] as string;

                                    if (arr != null)
                                    {
                                        //byteStr = string.Join(Environment.NewLine, arr); // todo
                                        //byteStr = arr.Last();
                                        byteStr = arr[0];
                                    }
                                    else if (s != null)
                    ...
                    if (pass == lastPass)
                    {
                        var hextext = string.Join(" ", byteArr);
                        ...
                        if (oldLabels.ContainsKey(counter_pad))
                        {
                            ...
                            for (var i = 0; i < leng; i++)
                            {
                                ...
                                {
                                    // if label is too long then put content on next line

                                    var skipCrlf = oldDidDrawLine || didDrawLine || (counter == origin);
                                    if (!skipCrlf)
                                    {
                                        theText.AppendLine();
                                    }

                                    theText.AppendLine($"{theLabel}:");
                                    if (i == leng - 1)
                                    {
                                        // last label, add space before content
                                        theText.Append(LEFT_MARGIN);
                                    }
                        ...
                        var line = (inputs.useLowerCase ? strtolower(mnemonic) : mnemonic) + " " + byteStr;
                ...
                if (pass < lastPass)
                {
                    oldLabels = new Dictionary<object, object>(labels);
                    oldPrgLabels = new Dictionary<object, object>(prgLabels);
            ...
            if (!string.IsNullOrWhiteSpace(inputs.targetPath) && !Directory.Exists(inputs.targetPath))
            {
                Directory.CreateDirectory(inputs.targetPath);
            }
            ...
            if (inputs.includeChr && headerInfo != null)
            {
                ...
                var incLine = LEFT_MARGIN + $".incbin \"{inputs.shortname}.chr\"";
                theText.Append(str_pad(incLine, 30 + labelLen) + " ; Include CHR-ROM\n");

                var chrOut = $"{inputs.shortname}.chr";
                file_put_contents(Path.Combine(inputs.targetPath, chrOut), rawChr);
                echo($"\nCHR-ROM exported as {chrOut}");


            }
            else if (inputs.includeChr)
            {
            ...
            var asmOut = $"{inputs.shortname}.asm";
            file_put_contents(Path.Combine(inputs.targetPath, asmOut), theText.ToString());


            var time_end = microtime(true);
            var time = Math.Round((time_end - time_start).TotalMilliseconds / 1000, 3);

            echo($"\nDisassembly {asmOut} generated in {time} seconds\n\n");
        }

        private static void RunOriginal(string[] argv)
        {
            string path = @"C:\working\disasm6\";
            ...
            startInfo.FileName = path + @"disasm6";
            //startInfo.WindowStyle = System.Diagnostics. ProcessWindowStyle.Hidden;
            startInfo.Arguments = _originalArgs + $"-t \"old-{argv[0]}\" ";
