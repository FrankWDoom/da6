...
namespace da6
{
    ...
    class Program
    {
        ...
        // defaults
        static int origin = 0x8000; // start of prg space
        static int endOfCode = 0xFFFA; // start of vectors
        ...
        static string[] _branches = new[] { "BCC", "BCS", "BEQ", "BMI", "BNE", "BPL", "BVC", "BVS", };
...
");
        ...
        private static void TracePrg(BankInfo bankInfo, List<byte[]> prgSlices, List<byte[]> cdlSlices, List<int> entryPoints, List<bool[]> codeMasks)
        {
        ...
        /// <summary>
        /// the big method that coordinates everything
        /// </summary>
        ...
        public static void Run(Inputs inputs)
        {
            ...
            if (!inputs.noDetect && headerInfo != null)
            {
                if (headerInfo.prg == 2) // 2 x 16K
                {
                    ...
                    if (headerInfo.mapper == 0 && IsDataRepeated(rawPrg)) // todo cnrom could also be this
                    {
            ...
            byte[] slice; // prg chunk
            byte[] sliceCdl; // cdl chunk
            ...
            // todo loop slices
            slice = prgSlices[0];
            sliceCdl = cdlSlices?.First();

            // todo this is temp values
            var bankInfo = new BankInfo() { Origin = origin, EndOfBank = endOfCode, };
            ...
            if (inputs.trace)
            {
                ...
                if (!string.IsNullOrWhiteSpace(inputs.traceFilename))
                {
                ...
                TracePrg(bankInfo, prgSlices, cdlSlices, entryPoints, codeMasks);
            }
            ...
            while (pass <= lastPass)
            {
                ...
                // todo figure out what to do with start overrides. may not make sense anymore.
                ...
                if (pass == 1)
                {
                    ...
                    if (inputs.codeStartOverride)
                    {
                        ...
                        cdlOffset += fileStart; // note: cdl does not have 16 byte header
                ...
                while (index < slice.Length && counter < fileLength)
                {
                    ...
                    if (inputs.usingMapper2 && headerInfo?.mapper == 2
                    ...
                    if (readBytes > 0)
                    {
                        ...
                        for (var i = 1; i <= readBytes; i++)
                        {
                            ...
                            if (isCounterLabel(counterNext, oldLabels)
                               || counterNext >= 0xFFFA || (counterNext >= fileLength)
                               || (operand == 0xff && _branches.Contains(mnemonic)) // branch into self
                    ...
                    if (readBytes == 2
                       ...
                       && addressingType > 0                        && addressingType < 9
                       // && addressingType != 9
                       && addressingType != 3)
                    {
                        isInvalid = true; // 1;
                    ...
                    if (addressingType > 0                       && isValidLabel(byteStr)
